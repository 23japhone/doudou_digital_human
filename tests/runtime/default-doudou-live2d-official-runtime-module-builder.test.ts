...
describe("default doudou official Live2D runtime module builder", () => {
  ...
  test("rejects framework mode when a declared texture slot has no file name", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "doudou-official-runtime-empty-texture-"));
    const originalFetch = globalThis.fetch;
    try {
      const sdkDir = path.join(tempRoot, "CubismSdkForWeb");
      const outputFile = path.join(tempRoot, "local_live2d_runtime", "default-doudou-official-runtime.mjs");
      await writeSyntheticCubismFrameworkSdk(sdkDir, { textureMode: "emptySlot" });
      ...
      const library = await loadDefaultDoudouLive2DPreviewLibrary(DEFAULT_DOUDOU_EXP3_FIXTURE_DIR);
      const runtimeModuleUrl = `${pathToFileURL(outputFile).href}?case=framework-empty-texture-${Date.now()}`;
      const host = createDoudouOfficialLive2DRendererHost({
        canvas: createFakeCanvas(),
        config: {
          publicEvidence: {
      ...
      expect(calls).toContain("CubismModelSettingJson.getTextureFileName:0");
      expect(calls).not.toContain("CubismRenderer_WebGL.loadShaders");
      expect(calls.some((call) => call === "CubismExpressionMotion.create")).toBe(false);
    } finally {
      globalThis.fetch = originalFetch;
...
`,
  ...
  await writeFile(
    ...
    `
const calls = () => globalThis.__doudouOfficialRuntimeFixtureCalls ?? [];
const textureMode = ${JSON.stringify(textureMode)};
export class CubismModelSettingJson {
  ...
  getTextureCount() {
    return textureMode === "emptySlot" ? 1 : 0;
  }
  getTextureFileName(index) {
    calls().push("CubismModelSettingJson.getTextureFileName:" + index);
    return "";
  }
  getLayoutMap(layout) {
