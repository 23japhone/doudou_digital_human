...
- `src/scripts/build-doudou-live2d-official-runtime-module.ts` is the Stage J local official runtime module builder. In its recommended sample mode, it uses the existing Vite toolchain to bundle a project-owned wrapper around the SDK sample `LAppModel`, then calls sample `loadAssets`, expression loading, `setExpression`, expression-manager `updateMotion`, `update`, and `draw` through the renderer host. Its framework mode remains available for lower-level Framework API checks against model setting parse, moc/model creation, expression motion creation/playback, `CubismMotionManager.updateMotion`, `model.update`, and `CubismRenderer_WebGL.doDrawModel`.
