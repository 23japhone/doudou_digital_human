...
- `npm run prepare:doudou-live2d-sample -- --sdk-dir <sdk-dir> [--sample Mao] [--out local_live2d_models/default-doudou-sample]` prepares a local official Sample Data model directory for SDK-pipeline testing. It copies `Samples/Resources/<sample>` from a developer-provided Cubism SDK for Web directory into ignored local storage, rewrites a `default-doudou.model3.json` that references the project-owned 12 default 兜兜 `.exp3.json` files, removes copied sample expression files only inside the new local output directory, and emits sanitized JSON without local paths. This is a pipeline diagnostic only and does not satisfy the final default 兜兜 model requirement.
