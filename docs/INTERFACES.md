...
- `npm --silent run smoke:doudou-live2d:official -- --sdk-dir <sdk-dir> --model-dir <model-dir> [--out local_live2d_runtime/default-doudou-official-runtime.mjs] [--mode sample|framework]` is the dedicated real local official Web SDK smoke gate. It requires explicit local SDK and default 兜兜 model paths, preflights the SDK/model layout through the Stage H resolver, builds the ignored local runtime module, runs the Electron runtime smoke with the official SDK/model/module environment, and emits sanitized pass/fail JSON without printing local paths. On success it summarizes the fixture and generated bundle official renderer evidence, including `rendererAssetProbe`, `runtimeModuleProbe`, `modelLoaded`, `expressionCount`, `expressionSwitches`, active emotion id, `updateCalls`, `drawCalls`, and `frameLoopAdvanced`. Environment fallbacks are `DOUDOU_CUBISM_WEB_SDK_DIR`, `DOUDOU_DEFAULT_DOUDOU_LIVE2D_MODEL_DIR`, and optional `DOUDOU_CUBISM_WEB_RUNTIME_MODULE`.
