...
The output report uses relative paths only and records non-sensitive metrics such as visible bounds and non-transparent pixel counts. The manual scoring template covers crop fit, mask silhouette, color preservation, edge clarity, and pet cuteness. A default stylizer parameter change must have completed scoring evidence for the candidate preset, explicit approval, and passing threshold scores before the default constants are changed. `npm run qa:stylizer:check -- <manual-scoring-template.json> <candidate-preset>` is the automation boundary for that gate: PR bots and pre-commit hooks should call it instead of reimplementing score rules. Generated QA outputs are local artifacts for inspection and should not include personal source images, secrets, prompts, or raw provider responses.
