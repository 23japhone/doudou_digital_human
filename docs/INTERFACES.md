...
- `npm --silent run build:doudou-live2d-runtime-module -- --sdk-dir <sdk-dir> --out local_live2d_runtime/default-doudou-official-runtime.mjs` builds a local-only ES module wrapper against the official Cubism SDK for Web Framework source. The CLI emits sanitized JSON with `outputFileName` and `Framework/src` status only; use `--silent` for shareable logs because npm can otherwise echo command arguments. The output module is intended for `DOUDOU_CUBISM_WEB_RUNTIME_MODULE` and should stay in ignored local storage.
...
- `src/scripts/build-doudou-live2d-official-runtime-module.ts` is the Stage J local official Framework wrapper builder. It uses the existing Vite toolchain to bundle a project-owned wrapper against `Framework/src`, and the wrapper calls official Framework APIs for model setting parse, moc/model creation, expression motion creation, `CubismMotionManager.updateMotion`, `model.update`, and `CubismRenderer_WebGL.doDrawModel`.
