...
- `npm run generate:pet -- <source-image-path> <output-bundle-dir>` validates and decodes a local PNG/JPEG source image, runs the default local 兜兜二次元数字人 adapter, then writes a deterministic `pet bundle v0.1` whose atlas must pass the shared 8-frame 兜兜 sprite QA gate.
...
- `npm run dev` launches the fixture directly in the Electron desktop runtime. This is the lower-level runtime entrypoint; use `Ctrl+C` in the terminal to stop it during development. The runtime always creates a system tray menu with `让兜兜回应（本次授权）`; choosing that item is the formal one-click authorization action, sends a fixed local intent text through the existing emotion-model arbitration boundary, and drives the current 兜兜 Live2D expression/motion runtime when a safe command is accepted. Add `DOUDOU_RUNTIME_TUNING=1` to show the developer-only motion tuning panel, or launch the runtime entrypoint with `--tuning`. Add `DOUDOU_EMOTION_TRIGGER_PANEL=1` or launch with `--emotion-panel` only for the legacy developer text panel. Provider-called, model, command, and apply evidence stay in structured smoke results only, never in user-visible menu or panel text. Optional launch presets are `DOUDOU_RUNTIME_RETREAT_DISTANCE`, `DOUDOU_RUNTIME_WATCH_MS`, and `DOUDOU_RUNTIME_RECOVERY_SPEED`. The motion tuning panel can copy the current settings as a one-line `DOUDOU_RUNTIME_TUNING=1 ... npm run dev` preset command, or save named local presets in the runtime user-data directory so different characters can keep separate rhythms. The current tuned defaults are 216px retreat distance, 680ms watch pause, and 280px/s recovery speed.
- `npm run replay:runtime` builds the main TypeScript entrypoints, reads the seven committed `fixtures/runtime/interaction_replay/*.json` files, runs the pure runtime replay runner, and emits one sanitized `runtime replay:` JSON summary with fixture ids, aggregate states/scenarios/emotion ids, max wariness, failure codes, and privacy status. It does not launch Electron and does not print step-by-step traces, local paths, prompts, provider payloads, or secrets.
- `npm run smoke:runtime` first runs the same replay fixture gate as a fast preflight, then runs Electron runtime negative cases and launches both the fixture bundle and a generated bundle before exiting after structured renderer smoke results, including primary-button drag window motion, proof that passive cursor contact did not move the overlay window, alpha-gated cursor-follow hit testing, observed runtime states, default 兜兜 emotion ids and scenarios, pet presentation envelope schema/reaction/stable-state evidence, pet performance governor schema/motion-budget/readability-catalog/readability-emotion/expression-priority/transition-tone evidence, emotion motion phases, motion tuning panel evidence, tray-menu 兜兜回应 evidence proving the formal menu item exists without requiring the flag-gated panel, motion direction, stop rebound, tap expression frames, runtime emotion-memory wariness from repeated alpha pokes, visual state class evidence, and the flag-gated Web Cubism renderer spike evidence for expression creation, expression switching, `updateMotion`, model update, draw calls, optional local official SDK/model resolver status, and optional external official runtime module host evidence. The readability catalog schema currently reported by smoke is `doudou.pet-performance-readability-catalog.v0.2`; unit tests bind that catalog to Live2D parameter vocabulary and manual QA standards. With `--synthetic-replay` or `DOUDOU_RUNTIME_SMOKE_SYNTHETIC_REPLAY=1`, the smoke also serializes the seven replay fixtures into a sanitized `doudou.runtime-smoke.synthetic-replay.v0.1` plan for the renderer and requires `syntheticReplay` evidence proving fixture ids, event count, DOM event dispatch, IPC bridge dispatch, explicit work session events, completion, and privacy status. When an official runtime module is loaded, the smoke requires the shared official evidence gate: fetched `model3.json`, loaded runtime module, loaded model, 12 expressions, multiple non-idle official expression emotions observed through desktop switches, no pending official expression switches, a non-idle active emotion, that switched expression applied after a successful frame, that the switched expression changed rendered Live2D canvas pixels, internal runtime lifecycle evidence for loading 12 expressions, setting at least two expressions, `updateMotion`, model update, and draw, visible Live2D canvas layer with nontransparent rendered pixels in the desktop window, advanced frame loop, and at least two host update/draw calls.
- `DOUDOU_EMOTION_TRAY_SMOKE_CONSENT=1 npm run smoke:runtime` is the explicit live acceptance path for the tray-menu 兜兜回应 entry. It requires the local `DOUDOU_EMOTION_MODEL_ENDPOINT`, `DOUDOU_EMOTION_MODEL_API_KEY`, and `DOUDOU_EMOTION_MODEL_ID` configuration, dispatches the tray menu request with consent enabled against the fixture bundle, requires provider-called plus command-applied evidence in structured smoke output, and still emits only user-facing sanitized status text. `DOUDOU_EMOTION_PANEL_SMOKE_CONSENT=1` remains accepted as a legacy alias.
