...
目标：先为 sprite atlas 建立轻量动作可读性规范，未来可迁移到 Live2D。当前已在 `src/runtime/performance-governor.ts` 落地 `doudou.pet-performance-readability-catalog.v0.1`，覆盖 12 个默认兜兜 emotion id，并把动作走廊、表情切换间隔和 transition tone 接进 `PetPerformancePlan`、interaction replay 和 runtime smoke evidence。
...
后续可以沿着同一目录继续补 Live2D 参数 vocabulary、各情绪的最小像素可读性 QA，以及不同角色资产的可替换 catalog preset。
...
1. 已采纳：`docs/DOUDOU_INTERACTION_STATE_BUS.md`、`src/runtime/presentation.ts` 与 `src/runtime/performance-governor.ts`
   已把桌宠交互状态总线、runtime-only affect core、reaction act、embodiment policy、`PetPresentationEnvelope` 和 readability catalog 落到兜兜 runtime/replay/smoke。`src/runtime/performance-governor.ts` 已把 AiG 的 `presentation envelope -> governor -> readability catalog -> renderer adapter` 映射成兜兜的动作幅度、节奏、表情切换间隔和可验证 evidence。
...
3. 更新 `docs/DEFAULT_DOUDOU_EMOTION_SPEC.md`
   在当前 runtime catalog 基础上继续补 emotion readability QA：每个 emotion id 的 Live2D 参数词汇、恢复规则、小尺寸可读性和人工评审标准。
