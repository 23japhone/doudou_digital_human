...
1. 已采纳：`docs/DOUDOU_INTERACTION_STATE_BUS.md` 与 `src/runtime/presentation.ts`
   已把桌宠交互状态总线、runtime-only affect core、reaction act、embodiment policy 和 `PetPresentationEnvelope` 落到兜兜 runtime/replay/smoke。后续已补 `src/runtime/performance-governor.ts`，把 AiG 的 `presentation envelope -> governor -> renderer adapter` 第二段映射成兜兜的动作幅度、节奏和表情限制。
