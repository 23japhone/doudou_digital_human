...
Phase 0: project framing, first desktop runtime vertical slice, local source-image intake, deterministic non-model stylized PNG adapter with QA-approved `bold_edges` defaults, scripted generation-adapter vertical slice, real image-to-character adapter design spike, cloud-adapter scaffold, preview/QA/deletion workflow, guided desktop UI with mock-cloud selection, OpenAI live provider scaffold behind explicit opt-in, and stylizer QA/manual-scoring automation with a change-aware default-parameter gate.
