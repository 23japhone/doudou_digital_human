...
Phase 0: project framing, first desktop runtime vertical slice, local source-image intake, scripted generation-adapter vertical slice, real image-to-character adapter design spike, and cloud-adapter scaffold.

Exit criteria: Electron loads a validated `pet bundle v0.1` fixture and generated bundle, renders idle animation in a transparent desktop window, generation flows through replaceable scripted/cloud adapter boundaries, cloud scaffold uses mocked provider tests with explicit opt-in, and validator/unit/build/runtime smoke checks pass.

## Next Vertical Slices

1. Harden click-through behavior beyond fallbackRect/alpha hit area if macOS manual testing shows input interference.
2. Add preview, QA, and deletion flows.
