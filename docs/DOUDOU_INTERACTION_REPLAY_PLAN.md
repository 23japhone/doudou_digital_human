...
Status: draft replay contract for next runtime slice
...
完成本文不要求立即实现 runner；但未来实现 runner、fixtures 和测试时，必须能直接从本文派生。
...
如果第一版 runner 还没有落地，本文就是 fixture 合同。后续代码实现时，不应把 replay fixtures 放到 flat root，也不应和 pet bundle fixtures 混在一起。
...
1. 新增 `src/runtime/interaction-replay.ts` 的纯函数 runner。
2. 新增六个 JSON fixtures 到 `fixtures/runtime/interaction_replay/`。
3. 新增 `tests/runtime/interaction-replay.test.ts`，先覆盖 pure runner，再考虑是否给 `npm run smoke:runtime` 接入同一批 fixture。
