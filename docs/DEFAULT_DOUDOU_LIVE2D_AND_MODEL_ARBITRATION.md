...
- Stage H resolver 只接受显式本地路径，不下载 SDK，不 vendor Core，不把绝对路径写入公开证据。它要求 SDK 目录包含 `Core/live2dcubismcore.js` 或 `Core/live2dcubismcore.min.js`、`Framework/src` 和官方 sample 的关键 TypeScript 源文件（包括 `lappmodel.ts`、`lapppal.ts`、`lapptexturemanager.ts` 等），模型目录包含 `default-doudou.model3.json` 以及其中引用的 `.moc3`、texture、expression、motion、physics/pose/display 文件。`model3.json` 内引用必须是安全相对路径，不能使用绝对路径、URL、反斜杠或 `..`。
