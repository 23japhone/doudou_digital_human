...
- Stage J builder 生成一个 project-owned wrapper module，而不是把 SDK 源码或模型资产复制进仓库。推荐 sample mode 用 Vite 从本机 `Samples/TypeScript/Demo/src/lappmodel.ts` 和 `Framework/src` bundle 必需类，调用 `LAppModel.loadAssets()`、把兜兜 12 个 `.exp3.json` request 注入 sample model、调用 `LAppModel.setExpression()`、`LAppModel.update()` 和 `LAppModel.draw()`。Framework mode 仍可直接用 `CubismModelSettingJson`、`CubismMoc`、`CubismExpressionMotion.create`、`CubismMotionManager.updateMotion`、`model.update()` 和 `CubismRenderer_WebGL.doDrawModel()` 做较低层检查。构建后会移除 bundler 生成的本机路径注释，CLI 只输出 `outputFileName`、`moduleFormat`、`Framework/src` 和 sample source 状态。
