...
- 官方 Sample Data SDK pipeline 预检可先运行：`npm run prepare:doudou-live2d-sample -- --sdk-dir <sdk-dir> --sample Mao --out local_live2d_models/default-doudou-sample`。该命令只复制开发者本机 SDK 包里的 `Samples/Resources/<sample>` 到 ignored 本地目录，把入口改写为 `default-doudou.model3.json`，并注入 12 个兜兜 expression 文件用于验证 SDK/Core/sample wrapper 链路；它不替代最终默认兜兜模型资产。
