...
- Stage K 新增真实本地 smoke gate：`npm --silent run smoke:doudou-live2d:official -- --sdk-dir <sdk-dir> --model-dir <model-dir> --mode sample`。该命令会构建 `local_live2d_runtime/default-doudou-official-runtime.mjs`，再用官方 SDK/model/module env 执行 `runtime-smoke`；`--silent` 用于避免 npm 在可分享日志中回显本机 SDK/model 路径。也可通过 `DOUDOU_CUBISM_WEB_SDK_DIR`、`DOUDOU_DEFAULT_DOUDOU_LIVE2D_MODEL_DIR` 和可选 `DOUDOU_CUBISM_WEB_RUNTIME_MODULE` 配置。
...
- `tests/scripts/doudou-live2d-official-smoke.test.ts` 校验 Stage K smoke gate 缺少 SDK/model 时失败、成功路径会构建 runtime module、注入 official SDK/model/module env 跑 `runtime-smoke`，并且输出不泄露本机路径。
