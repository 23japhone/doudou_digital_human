...
- Build local official Live2D runtime module: `npm --silent run build:doudou-live2d-runtime-module -- --sdk-dir <sdk-dir> --out local_live2d_runtime/default-doudou-official-runtime.mjs`
...
- Stage J local official runtime module builder tests live in `tests/runtime/default-doudou-live2d-official-runtime-module-builder.test.ts`. They use a synthetic Cubism Framework-shaped SDK fixture to require the builder to bundle a local external ES module, keep generated output free of local SDK paths, and prove the existing renderer host can drive official Framework-shaped calls for `CubismExpressionMotion.create`, `CubismMotionManager.updateMotion`, `model.update`, and `CubismRenderer_WebGL.doDrawModel`.
