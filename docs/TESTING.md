...
- Runtime smoke covers missing manifest, missing asset, unsupported schema, then launches the fixture and a generated bundle. Both require structured renderer evidence: bundle loaded, atlas loaded, drag moved, visible interaction frame, scale changed from wheel input, scale changed from pointer drag input, nontransparent canvas pixel, and idle animation advance.
