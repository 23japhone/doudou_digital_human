...
- Real local official smoke orchestration tests live in `tests/scripts/doudou-live2d-official-smoke.test.ts`. They require the dedicated smoke gate to fail fast when SDK/model paths are absent, pass `sample` or `framework` mode into the runtime module builder, run `runtime-smoke` with official SDK/model/module environment variables, and emit only sanitized JSON.
...
- Local official Live2D runtime module builder checks must keep generated wrapper modules in ignored local storage, strip bundler path comments, and report only sanitized filenames/status. A full real local smoke should use `npm --silent run smoke:doudou-live2d:official -- --sdk-dir <sdk-dir> --model-dir <model-dir> --mode sample`; the command builds `local_live2d_runtime/default-doudou-official-runtime.mjs`, then runs `runtime-smoke` with `DOUDOU_CUBISM_WEB_SDK_DIR`, `DOUDOU_DEFAULT_DOUDOU_LIVE2D_MODEL_DIR`, and `DOUDOU_CUBISM_WEB_RUNTIME_MODULE` set. Use `--mode framework` only when debugging below the sample `LAppModel` layer.
