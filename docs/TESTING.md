...
- Stage H local official SDK resolver tests live in `tests/runtime/default-doudou-live2d-official-sdk-resolver.test.ts`. They require missing local paths to return sanitized `not_configured`, complete local SDK/model fixtures to return public evidence plus renderer file URLs, and unsafe `model3.json` references that escape the model directory to be rejected without leaking absolute paths.
- Stage I external official renderer host tests live in `tests/runtime/default-doudou-live2d-official-renderer-host.test.ts` and `tests/runtime/runtime-live2d-csp.test.ts`. They require local Core loading before runtime module import, default model/expression handoff, expression switching, `update`/`draw` frame advancement, dedicated WebGL canvas wiring, and CSP that allows only local `file:` script/connect access rather than network origins.
- Shared official renderer smoke evidence tests live in `tests/runtime/default-doudou-live2d-official-smoke-evidence.test.ts`. They require the reusable evidence gate to reject loaded official runtime summaries that never switch expression or remain on `calm_idle`, accept complete loaded official runtime summaries, and parse both fixture/generated runtime smoke lines into sanitized summaries.
- Stage J local official runtime module builder tests live in `tests/runtime/default-doudou-live2d-official-runtime-module-builder.test.ts`. They use synthetic Cubism sample/framework-shaped SDK fixtures to require the builder to bundle a local external ES module, keep generated output free of local SDK paths, prove the recommended sample mode drives `LAppModel.loadAssets`, expression injection, `setExpression`, `update`, and `draw`, and prove the lower-level framework mode can still drive `CubismExpressionMotion.create`, `CubismMotionManager.updateMotion`, `model.update`, and `CubismRenderer_WebGL.doDrawModel`.
- Real local official smoke orchestration tests live in `tests/scripts/doudou-live2d-official-smoke.test.ts`. They require the dedicated smoke gate to fail fast when SDK/model paths are absent, preflight SDK/model layout before building, pass `sample` or `framework` mode into the runtime module builder, run `runtime-smoke` with official SDK/model/module environment variables, parse the fixture/generated bundle official renderer evidence, reject incomplete proof with sanitized failed-check names, and emit only sanitized JSON.
