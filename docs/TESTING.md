...
- Local official Live2D runtime module builder checks must keep generated wrapper modules in ignored local storage, strip bundler path comments, and report only sanitized filenames/status. A full manual smoke should first build `local_live2d_runtime/default-doudou-official-runtime.mjs` with `npm --silent run build:doudou-live2d-runtime-module -- --sdk-dir <sdk-dir> --out local_live2d_runtime/default-doudou-official-runtime.mjs --mode sample`, then run `npm run smoke:runtime` with `DOUDOU_CUBISM_WEB_SDK_DIR`, `DOUDOU_DEFAULT_DOUDOU_LIVE2D_MODEL_DIR`, and `DOUDOU_CUBISM_WEB_RUNTIME_MODULE` set.
