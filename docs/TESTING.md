...
- Real cloud adapter tests must mock provider calls by default and require explicit environment opt-in for any live-provider smoke.
...
- Real adapter changes must cover cloud confirmation gating, provider config failures, source normalization cleanup, provider error mapping, and no leakage of raw prompts, raw responses, tokens, or source paths.
