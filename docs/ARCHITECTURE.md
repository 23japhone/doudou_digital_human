...
- `src/generation/adapters/` for the default local 兜兜二次元数字人 adapter, deterministic local stylizer, scripted contract adapters, and real local/cloud adapters later.
...
- Use the 兜兜二次元数字人 adapter as the default non-model local MVP: resize to a 256x256 temporary PNG, derive small source-color accents from source pixels, render the project-owned 8-frame 兜兜 atlas, and require the shared 128px/256px sprite QA gate before writing the final bundle.
...
- Treat source-derived generated output and local stylizer previews as assets that may still preserve likeness. They are local-only and do not store the original source image, but they are not anonymization steps.
...
- Optional official Web SDK/model resolution also stays in runtime integration. Developers can point the runtime at a local Cubism SDK for Web directory and a local default 兜兜 model directory; main validates Core, the Framework source files directly needed by the official sample, official sample source layout, safe model references, and the exact 12 default 兜兜 expression references in `default-doudou.model3.json`, renderer probes the same file, and smoke reports sanitized availability. The repository still does not own SDK licensing, Core binaries, `.moc3`, textures, or model files.
- Optional official runtime module hosting also stays in runtime integration. Developers can point the renderer at a local ES module wrapper around the official sample/framework runtime; the renderer loads local Core, gives the wrapper a dedicated WebGL canvas, default model URLs, and expression load requests, waits for pending official expression switches to settle before smoke reporting, samples short in-memory canvas signatures around expression switches, and records only sanitized lifecycle counts/booleans including internal expression load/set, `updateMotion`, model update, draw evidence, pending-switch counts, and whether the switched expression changed rendered canvas pixels. The repository still does not own that wrapper or any licensed runtime assets.
- The local official runtime module builder is a developer tool, not production runtime code. It consumes a developer-provided Cubism SDK for Web directory, bundles a project-owned wrapper around official sample `LAppModel` plus `LAppPal` and Framework source classes, writes the generated ES module to an ignored local path such as `local_live2d_runtime/`, and reports only sanitized filenames/status. The recommended sample mode initializes `CubismFramework` before constructing the sample `LAppModel`, then drives sample `loadAssets`, waits for sample texture setup/CompleteSetup before expression loading, then drives expression injection/loading through `LAppModel._expressions.setValue` or `.set`, `setExpression`, `LAppPal.updateTime`, `update`, `draw`, and instrumented expression-manager `updateMotion`; the lower-level framework mode remains available for direct model setting, moc, expression motion creation/playback, motion manager, `CubismMotionManager.updateMotion`, and WebGL renderer checks. Core and model assets still come from explicit local paths at runtime.
