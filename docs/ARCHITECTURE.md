...
- Guided cloud mode currently supports `mock-provider` only, reads its mock API key from the app process environment, and still requires explicit upload confirmation before generation.
